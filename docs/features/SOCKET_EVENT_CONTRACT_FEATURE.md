...
알림 삭제까지 실시간 반영하려면 notification:delete 이벤트를 추가할 수 있다.

운영/배포 주의점:
기존 클라이언트가 레거시 이벤트를 쓰고 있어도 즉시 깨지지 않는다.

테스트 보강:
Socket.io 이벤트 alias, notification:new, notification:read, notification:readAll emit에 대한 통합 테스트를 추가할 수 있다.
